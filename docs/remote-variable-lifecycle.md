...
从 0.37.1 起，新协议在最终参数转换与 XML-RPC 校验结束后、调用代理方法前，
再次检查同一个准备截止时间，包含此前收集、等待 RPC 锁和序列化校验的耗时。
预算耗尽时明确报告“未发送请求”，不执行远程关键字。准备预算不会缩短业务
关键字原有的执行超时，也不会作为协议字段发送；旧协议调用流程保持兼容。
预算在处理阶段之间检查；Python 序列化、文件系统调用和业务关键字不是可强制
取消的操作，代理方法内部仍会进行 XML-RPC 报文编码和网络操作，因此该预算
不等于整个远程关键字的硬实时截止时间。
