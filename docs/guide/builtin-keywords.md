...
透传输入值，常用于自定义 DSL 关键字或流程中显式返回一个计算结果。

```python
# 直接返回一个简单值
value = [返回结果], 结果: 123
[断言], 条件: "${value} == 123", 消息: "返回值应保持不变"

# 返回对象或列表
payload = {"status": "ok", "id": 42}
result = [返回结果], 结果: ${payload}
[打印], 内容: "返回结果: ${result}"

# 在自定义 DSL 关键字中使用
function 计算总价 (单价, 数量) do
    total = 单价 * 数量
    [返回结果], 结果: ${total}
end
```
...
# 使用请求模板并覆盖部分配置
# 模板定义在变量配置的 http_templates 中，用户配置会与模板递归合并
[HTTP请求], 客户端: "default", 模板: "create_user", 配置: '''
    request:
        headers:
            X-Trace-ID: "${trace_id}"
        json:
...
[HTTP请求], 客户端: "default", 配置: '''
    ...
    url: /api/users
    request:
        json: "@file:data/user.json"
        headers:
            file_ref:
                path: "data/headers.yaml"
                type: "yaml"
        data:
            file_ref:
                path: "data/body.txt"
...
[HTTP请求], 客户端: "default", 配置: '''
    ...
    url: /api/users
    request:
        json: "@file_template:data/user_template.json"
'''
```
...
- 请求/头/数据字段支持通过`@file:`、`@file_template:`或`file_ref`引用外部文件
- `@file_template:`和`file_ref.template: true`会先执行变量替换，再根据文件类型解析
- 文件类型支持`auto`、`json`、`yaml`、`text`；`auto`会按扩展名识别`.json`、`.yaml`、`.yml`
...
```python
http_result = [HTTP请求], 客户端: "default", 保存响应: "raw_response", 配置: '''
    method: GET
    url: /api/users/1
    captures:
        user_id: ["jsonpath", "$.id"]
...
# 捕获值会写入上下文，也会出现在返回值 result 中
[打印], 内容: "用户ID: ${user_id}"
[打印], 内容: "返回结构中的用户ID: ${http_result['result']['user_id']}"
```

`side_effects.variables`记录本次注入上下文的捕获变量；`side_effects.context_updates.response`在设置`保存响应`时包含可序列化的响应摘要；`metadata`包含状态码、URL、响应耗时等元数据。

#### HTTP模板配置

可以在变量配置中定义`http_templates`，再通过`模板`参数复用：
...
调用时传入的`配置`会递归覆盖模板中的同名字段；嵌套字典会合并，普通值会以调用侧为准。

#### 文件引用格式

```yaml
request:
  json: "@file:data/payload.json"
  data: "@file_template:data/body.txt"
  headers:
    file_ref:
      path: data/headers.yaml
      type: yaml
...
如果引用文件不存在、JSON/YAML格式非法，关键字会抛出明确的异常。
...
- `matches`: 正则表达式匹配
- `and`/`or`: 按布尔值进行逻辑组合
- `not`: 对实际值取反，此时`预期值`可忽略
- `not in`: 不包含检查

**返回值：** 比较结果 (True/False)，比较失败时抛出`AssertionError`

**类型处理：**
- `实际值`和`预期值`如果是安全的数学表达式（如`"1 + 2"`），会先计算后比较
- 字符串形式的布尔值（`"true"`、`"false"`等）会转换为布尔值
- `matches`会将实际值转换为字符串后进行正则匹配；正则表达式非法时抛出`ValueError`
...
**返回值：** 通常为操作后的字符串；`split`未指定索引时返回列表，指定有效索引时返回对应元素。索引越界或不是非负数字时返回完整分割列表
...
**类型识别规则：**
- `number`接受真实数字，也接受能转换为数字的字符串
- `boolean`接受真实布尔值，也接受`"true"`/`"false"`字符串
- `list`接受真实列表，也接受合法 JSON 列表字符串
- `object`接受真实字典，也接受合法 JSON 对象字符串；兼容简单的 Python 字典字符串
- `null`接受`None`，也接受`"null"`或`"none"`字符串
...
**返回值：** 提取的值或值列表；未匹配时返回`None`，路径无效会抛出`ValueError`
...
**注意：** 当对象为`None`时返回`0`；对象不支持`len()`时会抛出异常。
...
**注意：** `数据`可以是列表等可迭代对象，也可以是单个标量值。空列表且未提供`默认值`时会抛出异常；`数据`为`None`且未提供`默认值`时也会抛出异常。
...
**注意：** 该关键字直接使用 Python `str()`语义，`None`会转换为字符串`"None"`。
...
**注意：** 该关键字直接使用 Python `int()`语义。浮点数会截断小数部分；`值`为`None`或无法转换时会抛出异常。
...
**注意：** 该关键字直接使用 Python `bool()`语义，不会把字符串`"false"`特殊转换为`False`。因此非空字符串（包括`"false"`、`"0"`）都会得到`True`；需要判断布尔字符串类型时请使用`类型断言`。
...
# 生成包含字母、数字和标点符号的字符串
all_str = [生成随机字符串], 长度: 12, 类型: "all"
[打印], 内容: "全部字符随机串: ${all_str}"
```
...
**注意：** 时区非法时会回退为本地时间；格式化字符串非法时会回退为`%Y-%m-%d %H:%M:%S`。
...
- `returncode`: 命令退出码；超时或执行异常时为`-1`
- `stdout`: 标准输出；当`捕获输出`为`false`时为空字符串
- `stderr`: 标准错误；超时时包含超时说明，执行异常时包含异常信息

**注意：** 该关键字通过系统 shell 执行命令。命令字符串应来自可信输入，避免把未校验的外部数据拼接到命令中。
...
| 打印 | 输出信息 | 内容 | 无 |
| 等待 | 延时执行 | 无 | 无 |
| 断言 | 条件验证 | 条件 | True/失败抛异常 |
| 返回结果 | 结果返回 | 结果 | 指定结果 |
| **HTTP测试** |
| HTTP请求 | API测试 | 配置 | result为捕获值字典（可选保存响应） |
| **数据处理** |
| 数据比较 | 值比较 | 实际值, 预期值 | 布尔值/失败抛异常 |
| JSON提取 | JSON数据提取 | JSON数据, JSONPath | 提取值 (未匹配返回None) |
...
# 转换为布尔值使用 Python bool() 语义
bool_text = [转换为布尔值], 值: "false"  # 返回 True，因为非空字符串为 True
```
...
```python
result = [HTTP请求], 客户端: "default", 保存响应: "response", 配置: '''
    method: GET
    url: /api/users/1
    captures:
        user_id: ["jsonpath", "$.id"]
...
# 返回值本身是结构化字典，捕获值在 result 字段中
[打印], 内容: "返回结构: ${result}"
[打印], 内容: "状态码: ${result['metadata']['status_code']}"
```

远程关键字模式下也依赖这个结构传递`result`、`side_effects`和`metadata`，因此不要假设`HTTP请求`直接返回响应对象；需要响应对象时使用`保存响应`。
