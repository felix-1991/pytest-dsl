...
适用于任意语句块（不仅是HTTP）：按次数/间隔重放，直到成功或条件满足。
...
- `retry <次数>` 必填，可用表达式或变量。
- `every <间隔>` 可选，缺省使用执行器默认间隔。
- `until <条件>` 可选，条件为真时提前结束；未写时只要块体执行成功即视为完成。
- 块内抛异常/断言失败会触发下一次重试，次数耗尽后抛出最后一次的错误。
...
> 参数默认值支持字符串、数字、布尔值和空值字面量（`null`/`None`）。
...
- 逻辑运算：`and`、`or`、`not`
- 算术运算：`+` `-` `*` `/` `%`，括号可调整优先级
- 一元负号：支持负数字面量和负号表达式，如 `-1`、`-(1 + 2)`
- 布尔字面量：`True`、`False`
- 列表/字典字面量：`[1, 2]`、`{"a": 1, "b": 2}`

> 运算符优先级从低到高：`,` < `or` < `and` < `not` < `in/not in` < 比较运算 < 加减 < 乘除模 < 一元负号。必要时使用括号明确顺序。
