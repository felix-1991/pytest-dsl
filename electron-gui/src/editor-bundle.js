var CM6 = (() => {
      ...
      mount: () => {
        if (config2.focus) {
          ...
          if (typeof config2.focus == "string")
            focus2 = content2.querySelector(config2.focus);
          else
            focus2 = content2.querySelector("input") || content2.querySelector("button");
          if (focus2 && "select" in focus2)
            focus2.select();
          else if (focus2 && "focus" in focus2)
            focus2.focus();
        }
      }
    };
  ...
  var HistEvent = class _HistEvent {
    ...
    static selection(selections2) {
      return new _HistEvent(void 0, none2, void 0, void 0, selections2);
    }
  };
  ...
  function addMappingToBranch(branch, mapping) {
    if (!branch.length)
      return branch;
    let length = branch.length, selections2 = none2;
    while (length) {
    ...
    return selections2.length ? [HistEvent.selection(selections2)] : none2;
  }
  function mapEvent(event, mapping, extraSelections) {
    let selections2 = conc(event.selectionsAfter.length ? event.selectionsAfter.map((s) => s.map(mapping)) : none2, extraSelections);
    if (!event.changes)
  ...
  var CompletionTooltip = class {
    ...
    addInfoPane(content2, completion) {
      ...
      if (content2.nodeType != null) {
        ...
        let { dom, destroy: destroy2 } = content2;
        wrap.appendChild(dom);
        this.infoDestroy = destroy2 || null;
      }
  ...
  var Config = /* @__PURE__ */ Facet.define({
    combine: (c) => {
      const configs = [];
      for (let config2 of c) {
        if (!config2) {
          continue;
        }
  ...
  var Theme$1 = /* @__PURE__ */ EditorView.theme({
  ...
  var OverlayView = /* @__PURE__ */ ViewPlugin.fromClass(class {
    constructor(view) {
      this.view = view;
  ...
  var Overlay = [Theme$1, OverlayView];
  ...
  var LinesState = /* @__PURE__ */ StateField.define({
  ...
  var TextState = class extends LineBasedState {
    ...
    updateImpl(state, changes) {
      ...
      const parser = (_a2 = state.facet(language)) === null || _a2 === void 0 ? void 0 : _a2.parser;
      const tree = parser ? parser.parse(docToString, treeFragments) : void 0;
      this._previousTree = tree;
      const highlighter = {
        style: (tags2) => highlightingFor(state, tags2)
      };
      let highlights = [];
      if (tree) {
        ...
        highlightTree(tree, highlighter, (from, to, tags2) => {
          highlights.push({ from, to, tags: tags2 });
      ...
      for (const [index, line] of state.field(LinesState).entries()) {
        ...
        for (const span of line) {
          ...
          while (!highlightPtr.done && highlightPtr.value.from < span.to) {
            const { from, to, tags: tags2 } = highlightPtr.value;
            if (to < position) {
            ...
            spans.push({ text: docToString.slice(start, end), tags: tags2 });
            position = end;
            if (to > end) {
              break;
            }
    ...
    getFontInfo(tags2) {
      const cached = this._fontInfoMap.get(tags2);
      if (cached) {
        return cached;
      }
      const mockToken = crelt("span", { class: tags2 });
      ...
      this._fontInfoMap.set(tags2, result);
  ...
  var Theme = /* @__PURE__ */ EditorView.theme({
    "&": {
      height: "100%",
      overflowY: "auto"
    },
  ...
  var WIDTH_RATIO = 6;
  var minimapClass = /* @__PURE__ */ ViewPlugin.fromClass(class {
    constructor(view) {
      this.view = view;
      this.text = text(view);
    ...
    canvasStartAndEndIndex(context, lineHeight) {
      ...
      const lineCount2 = this.view.state.field(LinesState).length;
      const totalHeight = pTop + pBottom + lineCount2 * lineHeight;
  ...
  var showMinimap = /* @__PURE__ */ Facet.define({
    combine: (c) => {
      var _a2;
  ...
  var yamlLanguage = StreamLanguage.define({
    name: "yaml",
    token(stream) {
      if (stream.sol() && stream.eatSpace() && stream.eat("#")) {
        stream.skipToEnd();
  ...
  var pythonLanguage = StreamLanguage.define({
    name: "python",
    token(stream) {
      if (stream.sol() && stream.eatSpace() && stream.eat("#")) {
        stream.skipToEnd();
  ...
  var markdownLanguage = StreamLanguage.define({
    name: "markdown",
    token(stream) {
  ...
  var setDebugStateEffect = StateEffect.define();
  var debugStateField = StateField.define({
    create() {
  ...
  var debugLineDecorations = StateField.define({
    create() {
      return Decoration.none;
    },
    update(_value, tr) {
      ...
      if (ds.debugSelection) {
        ...
        const to = Math.min(doc2.lines, ds.debugSelection.endLine);
        for (let i = from; i <= to; i++) {
          builder.push(Decoration.line({ class: "is-debug-selected" }).range(doc2.line(i).from));
        }
      }
      if (ds.debugStartLine && ds.debugStartLine >= 1 && ds.debugStartLine <= doc2.lines) {
        builder.push(Decoration.line({ class: "is-debug-start" }).range(doc2.line(ds.debugStartLine).from));
      }
      if (ds.currentDebugLine && ds.currentDebugLine >= 1 && ds.currentDebugLine <= doc2.lines) {
        builder.push(Decoration.line({ class: "is-debug-current" }).range(doc2.line(ds.currentDebugLine).from));
      }
  ...
  var DebugStartMarker = class extends GutterMarker {
    toDOM() {
  ...
  var debugGutterMarkers = StateField.define({
    create() {
      return [];
    },
    update(_value, tr) {
  ...
  var bridge = {
    ...
    getSelection() {
      ...
      const content2 = state.doc.sliceString(fromPos, toPos);
      if (!content2.trim()) return null;
  ...
  var getSelection2 = bridge.getSelection.bind(bridge);
  var getKeywordUnderCursor = bridge.getKeywordUnderCursor.bind(bridge);
  var insertText = bridge.insertText.bind(bridge);
